...
export const tools = [
  ...
  },
  {
    ...
    **필수 입력 파라미터:**
    ...
    - \`productAreaCd\` (지역코드): 상품이 속한 지역정보를  \`getDetailCommonCodeByQuery\`을 사용하여 정확한 지역 코드를 조회하여 입력해야 합니다. 만약 코드를 찾을 수 없거나 특정 지역을 지정하지 않는 경우 'A0'을 기본값으로 사용합니다.

    **선택 입력 파라미터:**
    - \`saleProductCode\` (판매상품코드): 특정 판매 상품을 조회할 때 사용하는 고유 코드.
    - \`reservationCode\` (예약코드): 특정 예약과 관련된 상품을 조회할 때 사용하는 코드.
    - \`productAttributeCode\` (상품속성코드): 상품의 속성(예: '패키지', '자유여행', '골프')을 나타내는 코드. \`retrieveAreaCode\`을 사용하여 사용자 질의에 맞는 코드값을 조회하여 입력합니다.
