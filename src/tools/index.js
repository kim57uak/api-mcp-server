...
export const tools = [
  ...
  },
  {
    ...
    **필수 입력 파라미터:**
    - \`startDate\` (시작일/출발일): 상품 검색을 위한 시작 날짜 (YYYYMMDD 형식).s
    - \`endDate\` (종료일/도착일): 상품 검색을 위한 종료 날짜 (YYYYMMDD 형식).
    - \`productAreaCd\` (지역코드): 상품이 속한 지역정보를  \`retrieveAreaCode\`을 사용하여 정확한 지역 코드를 조회하여 입력해야 합니다. 만약 코드를 찾을 수 없거나 특정 지역을 지정하지 않는 경우 'A1'을 기본값으로 사용합니다.
