// src/tools/retrieveComPkgProdStrtrPromotionTool.js
import { z } from "zod";
 ...
 */
export const retrieveComPkgProdStrtrPromotionTool = {
  name: "retrieveComPkgProdStrtrPromotion",
  description:
    "OLS에서 프로모션 코드('02') 리스트를 조회합니다. (상품구분 : 01 프로모션 : 02 테마 : 03)",
  inputSchema: z.object({}), // 이 Tool은 입력을 받지 않습니다.
  async handler() {
    const toolName = "retrieveComPkgProdStrtrPromotionTool";
    logger.info(`Executing ${toolName}. No input parameters.`);

    try {
      const result = await packageService.retrieveComPkgProdStrtrPromotion();
      logger.info(
