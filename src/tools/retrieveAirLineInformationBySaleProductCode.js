...
export const retrieveAirLineInformationBySaleProductCodeTool = {
  name: "retrieveAirLineInformationBySaleProductCode",
  ...
  async handler({ saleProductCode, departureDay }) {
    ...
    const functionName = "retrieveAirLineInformationBySaleProductCodeTool.handler";
    logger.info(
      `Executing ${functionName} with saleProductCode: ${saleProductCode}, depDay: ${departureDay}`
    );
    try {
      const result = await packageService.retrieveAirLineInformationBySaleProductCode({
        saleProductCode,
        departureDay,
      });
      logger.info(
