// src/tools/retrieveComPkgProdStrtrThemeTool.js
import { z } from "zod";
 ...
 */
export const retrieveComPkgProdStrtrThemeTool = {
  name: "retrieveComPkgProdStrtrTheme",
  description:
    "OLS에서 테마 코드('03') 리스트를 조회합니다. (상품구분 : 01 프로모션 : 02 테마 : 03)",
  inputSchema: z.object({}), // 이 Tool은 입력을 받지 않습니다.
  async handler() {
    const toolName = "retrieveComPkgProdStrtrThemeTool";
    logger.info(`Executing ${toolName}. No input parameters.`);

    try {
      const result = await packageService.retrieveComPkgProdStrtrTheme();
      logger.info(
