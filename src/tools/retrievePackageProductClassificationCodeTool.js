// src/tools/retrievePackageProductClassificationCodeTool.js
import { z } from "zod";
import { packageService } from "../services/packageService.js";
import logger from "../utils/logger.cjs"; // 경로 수정: ../../utils/logger.cjs -> ../utils/logger.cjs

/**
 * @tool retrievePackageProductClassificationCodeTool
 ...
 */
export const retrievePackageProductClassificationCodeTool = {
  name: "retrievePackageProductClassificationCode",
  description:
    "OLS에서 상품 구분 코드('01') 리스트를 조회합니다. (상품구분 : 01 프로모션 : 02 테마 : 03)",
  inputSchema: z.object({}), // 이 Tool은 입력을 받지 않습니다.
  async handler() {
    const toolName = "retrievePackageProductClassificationCodeTool";
    logger.info(`Executing ${toolName}. No input parameters.`);

    try {
      const result =
        await packageService.retrievePackageProductClassificationCode();
      logger.info(
