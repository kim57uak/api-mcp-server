...
export const retrieveExpenseInformationBySaleProductCodeTool = {
  name: "retrieveExpenseInformationBySaleProductCode",
  ...
  async handler({ saleProdCd, depDay }) {
    ...
    try {
      const result = await packageService.retrieveExpenseInformationBySaleProductCode({
        saleProdCd,
        depDay,
      });
      logger.info(
